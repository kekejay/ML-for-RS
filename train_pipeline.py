...
from objective import objective_ml_model, objective_dl_model,fixed_ml_hyperparams
...
def generate_or_load_folds(X, y, groups, n_splits, save_dir):

    Path(save_dir).mkdir(parents=True, exist_ok=True)
    fold_files = [f"{save_dir}/fold_{i}.pkl" for i in range(n_splits)]
    
    if all(os.path.exists(f) for f in fold_files):
        print(f"[✓] The saved {n_splits}-fold data has been detected and will be loaded directly.")
        # 读取并返回folds列表
        folds = []
        for f in fold_files:
    ...
    # 否则重新划分并保存
    print(f"[✳] Generate the {n_splits} fold for cross-validation and save...")
...
def run_model_pipeline(
    ...
    save_yaml(study.best_params, 
              f"{save_path}/best_search_{model_name}_params.yaml")

    final_params = study.best_params.copy()

    default_space = fixed_ml_hyperparams(model_name,seed)
